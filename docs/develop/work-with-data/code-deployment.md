...
When working with Ignite 3, you may need to deploy user code to cluster nodes so that it can be executed across the cluster, as shown in the [Distributed Computing](./compute) section.
...
You can manage deployment units using either [CLI](/tools/cli-commands) commands or the [REST API](https://ignite.apache.org/releases/ignite3/openapi.yaml). Both methods provide the same functionality for deploying, listing, and undeploying code.
...
- For additional details see the corresponding [API documentation](https://ignite.apache.org/releases/ignite3/openapi.yaml).
...
| Parameter | Description |
|-----------|-------------|
| statuses | Filter units by status.<br/>- `UPLOADING` - the unit is being deployed to the cluster<br/>- `DEPLOYED` - the unit is deployed to the cluster and can be used<br/>- `OBSOLETE` - the command to remove unit has been received, but it is still used in some jobs<br/>- `REMOVING` - the unit is being removed<br/><br/>If not specified, deployment units in all statuses will be returned. |
...
| statuses | query | Statuses of the deployment units to return. Possible values:<br/>- `UPLOADING` - the unit is being deployed to the cluster<br/>- `DEPLOYED` - the unit is deployed to the cluster and can be used<br/>- `OBSOLETE` - the command to remove unit has been received, but it is still used in some jobs<br/>- `REMOVING` - the unit is being removed<br/><br/>If not specified, deployment units in all statuses will be returned. |
